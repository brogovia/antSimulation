...
public final class Environment implements FoodGeneratorEnvironmentView, AnimalEnvironmentView, AnthillEnvironmentView,
	...
	public Food getClosestFoodForAnt(AntWorker antWorker) {
		...
		if (closestFood == null || closestFood.getPosition().toricDistance(antWorker.getPosition()) > getConfig()
	...
	public boolean dropFood(AntWorker antWorker) {
		...
		if (antWorkerAnthill == null || antWorkerAnthill.getPosition()
				.toricDistance(antWorker.getPosition()) > getConfig().getDouble(ANT_MAX_PERCEPTION_DISTANCE)) {
			return false;
		}
	...
	public double[] getPheromoneQuantitiesPerIntervalForAnt(ToricPosition position, double directionAngleRad,
		...
		for (Pheromone pheromone : pheromones) {
			if (!pheromone.isNegligible() && pheromone.getPosition().toricDistance(position) <= getConfig()
					.getDouble(ANT_SMELL_MAX_DISTANCE)) {
				...
				for (int i = 0; i < angles.length; i++) {
					if (closestAngleFrom(angles[i], beta) < (closestAngleFrom(angles[closestIndice], beta)))
						closestIndice = i;
				}
	...
	public List<Animal> getVisibleEnemiesForAnimal(Animal from) {
		double dmax = getConfig().getDouble(ANIMAL_SIGHT_DISTANCE);
		return this.animals.stream()
				.filter(a -> (a.getPosition().toricDistance(from.getPosition()) <= dmax) && a.isEnemy(from))
